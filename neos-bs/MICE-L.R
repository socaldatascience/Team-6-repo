...
load("C:/Users/neoda/Cal State Fullerton/Jaynes, Jessica - CSUF CHOC Summer 2022 Research/Group2/cfCOVIDgroup6.RDATA") # LAPTOP

load("C:/Users/neoda/Cal State Fullerton/Jaynes, Jessica - Group2/cfCOVIDgroup6.RDATA") #Desktop
...
choc <- d %>% 
  ...
  dplyr::select(personid, encounterid, COVIDseverity, age_at_encounter, gender, 
...
predM[, c("personid", "gender")]=0

meth[c("COVIDseverity", "age_at_encounter")]= ""
...
#--- FORMATONG AND JOINING DATA ------------------------------------------------

imputed.choc <- imputed.choc %>% 
  dplyr::select(-gender, -COVIDseverity, -age_at_encounter) %>% 
  mutate(
    tempC = admit_tempC,
...
choc.full <- choc %>% 
  left_join(imputed.choc, by = "personid")
...
choc.full <- choc.full %>% 
  mutate(age_group = as.factor(case_when(age_at_encounter <= 11 ~ "0 - 11",
                                    age_at_encounter > 11 & 
...
polr(formula = COVIDseverity ~ 
       comorb_resp_failure_J96 + 
       comorb_bronchiectasis_J47 + 
       comorb_pneumothorax_J93 + 
       comorb_malnutrition + 
       comorb_liver_disease_K70_K77 +
       tempC + 
       heart_rate+ 
       resp_rate + 
       SPO2 + 
       ...
       ALT + 
       LDH, 
    data = choc.full, Hess = T)

e <- seq(1, 4662)
f <- sample(e, 4195, replace = F)

mod.train <- choc.fitted[f,]
mod.test <- choc.fitted[-f,]

# comorb_resp_failure_J96 + comorb_bronchiectasis_J47 + 
...
#   lymphocytes + ALP + ALT + LDH

fill.ord <- polr(formula = COVIDseverity ~ 
                   comorb_bronchiectasis_J47 + 
                   comorb_malnutrition +
                   ...
                   , data = mod.train, Hess = T)

fill.ord_pred <- predict(fill.ord, newdata = mod.test, probability = TRUE)

table(mod.test$COVIDseverity, fill.ord_pred)

fill.ord_pred_levels <- factor(if_else(fill.ord_pred > 0.5, "2", "1", "0"), levels = c("0", "1", "2"))

multiROC <- multiclass.roc(mod.test$COVIDseverity, as.numeric(as.character(fill.ord_pred)))
...
glm(bin_COVIDseverity ~ comorb_bronchiectasis_J47 + comorb_malnutrition +
      ...
      comorb_resp_failure_J96*comorb_malnutrition, data = choc.fitted, 
    family = binomial(link = "logit"))

##Optimized Model
glm(bin_COVIDseverity ~ 
      comorb_bronchiectasis_J47 + 
      comorb_malnutrition +
      comorb_other_GI_notLiver_K_excludesK70K77 +
      comorb_chronic_kidney_disease_N18 + 
      ...
      leukocytes_cat*tempC_cat +
      comorb_bronchiectasis_J47*spo2_cat
    ,data = choc.fitted, family = binomial(link = "logit"))


#--- Testing Optimized Model ---------------------------------------------------

e <- seq(1, 4662)
f <- sample(e, 3729, replace = F)
